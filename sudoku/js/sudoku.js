...
The code was taken from Sudoku.js and was adapted for my purpose.
...
class Sudoku {
    static DIFFICULTY = [];
    static root = root.sudoku;

    static generate(difficulty, unique) {
        return root.generate(difficulty, unique)
    }

    static solve(board, reverse) {
        return root.solve(board, reverse);
    }

    static get_candidates(board) {
        return root.get_candidates(board);
    }

    static board_string_to_grid(board_string) {
        return root.board_string_to_grid(board_string);
    }

    static board_grid_to_string(board_grid) {
        return root.board_grid_to_string(board_grid);
    }

    static validate_board(board) {
        return root.validate_board(board);
    }
